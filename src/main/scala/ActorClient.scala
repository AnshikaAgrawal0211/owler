...
case class Act(companyName: String) {
  ...
  val responseFuture: Future[HttpResponse] = Http().singleRequest(
    HttpRequest(uri = "https://www.owler.com/company/" + companyName)
...
class ActorClient extends Actor {
  def receive = {
    case "hitowler" =>
      val inputCompanyName = scala.io.StdIn.readLine()
      Act(inputCompanyName)
    case _ =>
...
object Company extends App {
  val system = ActorSystem("hiturl")
  val hit = system.actorOf(Props[ActorClient])
  hit ! "hitowler"
}
